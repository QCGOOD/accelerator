<template>
  <div class="want-sign">
    ...
    <div class="btn-pay" @click="save()" v-if="model.advanceCharge && model.advanceCharge > 0">
      <div class="price">预付款 <span>{{ model.advanceCharge | formatPrice }}元</span></div>
...
export default {
  ...
  methods: {
    ...
    apiSaveOrder(query) {
      ...
      this.$http.json("/courseSignTemp/sign", query)
        .then(res => {
          ...
          //   // 开启审核后
          //   this.jumpPage("/curriculum/success?type=1");
          if (this.model.advanceCharge && this.model.advanceCharge > 0) {
            // 要给钱
            this.apiPayOrder(res.data.message);
          } else {
            // 完美无暇
            this.jumpPage("/curriculum/success?type=0");
          }
        })
        .catch(err => {
    ...
    apiPayOrder(orderId, orderType = 2) {
      this.$http.post("/pay", { orderId, orderType, payReturnUrl:`${this.$ROOTURL}/mobile/#/curriculum/success?type=0` }).then(res => {
