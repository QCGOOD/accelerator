      ...
      <div class="item-box my">
        ...
        <div class="content white">
          ...
          <div class="c-img-box" v-if="feel.imgUrl">
            <div 
              v-if="item" 
              v-for="item in feel.imgUrl" :key="item"
              class="img" 
              :class="{'one-img': feel.imgUrl.length == 1, 'two-img': feel.imgUrl.length == 2, 'more-img': feel.imgUrl.length > 2 }" 
              @click="previewImage(imgHost+item, feel.imgUrl)">
              <img :src="`${imgHost}${item}${cutParam4}`" alt="">
            </div>
          </div>
        </div>
        <div class="btn">
  ...
  methods: {
    ...
    getlessonFeel() {
      this.$http.get('/partakeSense/get', {id: this.$route.query.id}).then(res => {
        ...
        // 有优惠券
        if (res.data.data.reviewSummary.couponRuleId) {
